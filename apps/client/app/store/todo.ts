...
import { fetchAddTodo, fetchDeleteTodo, fetchGetDeletedTodoList, fetchGetTagList, fetchGetTodoById, fetchGetTodoList, fetchUpdateTodo } from '~/services/todo'
...
export const useTodoStore = defineStore('todo', () => {
  ...
  function updateTodoList(type: 'delete', id: string): void
  function updateTodoList(type: 'add' | 'update' | 'delete', todoOrId: Todo | string) {
    if (type === 'add' && typeof todoOrId !== 'string') {
      // 按照 created_at DESC 排序插入到正确位置
      insertTodoInSortedOrder(todos.undone.list, todoOrId)
    }
      ...
      (Object.keys(todos) as (keyof typeof todos)[]).forEach((key) => {
        const index = todos[key as keyof typeof todos].list.findIndex(item => item.id === todoOrId)
        if (index !== -1) {
